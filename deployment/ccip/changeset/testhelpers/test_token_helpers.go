...
import (
	...
	token_governor "github.com/smartcontractkit/ccip-contract-examples-internal/gobindings/generated/latest/token_governor"
...
func DeployTokenGovernor(t *testing.T, e cldf.Environment, selector uint64, token common.Address) (common.Address, *token_governor.TokenGovernor) {
	...
	tg_address, tx, contract, err := token_governor.DeployTokenGovernor(chain.DeployerKey, chain.Client, token, big.NewInt(0), chain.DeployerKey.From)
	require.NoError(t, err)
	_, err = chain.Confirm(tx)
	require.NoError(t, err)
	return tg_address, contract
}
